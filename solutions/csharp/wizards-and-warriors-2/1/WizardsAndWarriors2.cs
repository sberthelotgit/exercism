static class GameMaster
{
    ...
    => $"You're traveling to your destination {(TravelMethod.Horseback == travelMethod ? "on horseback" : "by walking")}.";

    public static string Describe(Character character, Destination destination, TravelMethod travelMethod = TravelMethod.Walking)
   => string.Join(' ', new string[] { Describe(character), Describe(travelMethod), Describe(destination) });
}
